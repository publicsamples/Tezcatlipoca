...
#include "src/filterBank.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct filterBankMetaData {
		SN_NODE_ID("filterBank");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using filterBank = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _filterBank, filterBankMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
